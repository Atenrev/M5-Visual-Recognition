...
class SKNNWrapper:
    ...
    def fit(self):
        if self.state_variables['built']: raise AssertionError('Cannot fit a built KNN')
        else: self.state_variables['built'] = True
    ...
    def retrieve_by_idx(self, *args, **kwargs):
        raise NotImplementedError('Retrieval by ID is not implemented for sklearn KNN')

    def retrieve_by_vector(self, vector, n = None, **kwargs):
        if not (n is None): warnings.warn('SKLearn retrieval receives the K parameter on the constructor. Ignoring N kwarg...')
