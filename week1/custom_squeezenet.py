...
class SmallSqueezeNetCNN(nn.Module):
    def __init__(self, num_classes=8):
        super(SmallSqueezeNetCNN, self).__init__()
        ...
        self.conv10 = nn.Conv2d(256, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x41 = self.batch_norm_fire21(x41)
        x41 = self.activation(x41)

        x42 = self.fire4_expand2(x4)
        x42 = self.batch_norm_fire22(x42)
        ...
        x5 = self.fire3_squeeze(x4)
        x51 = self.fire3_expand1(x5)
        x51 = self.batch_norm_fire31(x51)
        x51 = self.activation(x51)

        x52 = self.fire3_expand2(x5)
        x52 = self.batch_norm_fire32(x52)
        ...
        softmax = self.softmax(global_avgpool10)
